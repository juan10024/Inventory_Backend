...
public class CategoryServicesImpl implements ICategoryService {
		...
		public ResponseEntity<CategoryResponseRest> update(Category category, Long id) 
		{
			...
			{ 
				...
				if(categorySearch.isPresent()) 
				{
					...
					if(categoryToUpdate != null ) 
					{
						...
						response.getCategoryResponse().setCategory(list); 
						response.setMetadata( " Respuesta bien " , " 00 ", " La categoría ha sido actualizada con éxito " ); 
					} 
					else 
					{
						response.setMetadata( " Respuesta mal " , " -1 ", " Error, la categoría no ha sido actualizada " );  
						return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.BAD_REQUEST);  
					}
			...
			{
				response.setMetadata( " Respuesta mal " , " -1 ", " Error al actualizar la categoría ");
				e.getStackTrace();
				return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);	
			}
		...
		public ResponseEntity<CategoryResponseRest> deleteById(Long id) { 
			...
			{ 
				...
				response.setMetadata( " Respuesta ok " , " 00 ", " La categoría ha sido eliminada exitosamente ");
			} 
			catch(Exception e) 
			{
				response.setMetadata( " Respuesta mal " , " -1 ", " Error al eliminar la categoría ");
				e.getStackTrace();
				return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);	
			}
